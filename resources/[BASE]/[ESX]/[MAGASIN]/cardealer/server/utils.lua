...
function getAllCars()
    local result = MySQL.Sync.fetchAll("SELECT * FROM vehicles_offroad ", {})
	if result[1] ~= nil then
		local veh = result
		local vehicles_offroad = {}

		for k,v in pairs(veh) do
			if Config.Debug then
				print(" Modelis: " .. veh[k].model .. " Pavadinimas: " .. veh[k].name .. " Kaina: " .. veh[k].price)
			end
			table.insert(vehicles_offroad, {
				modelo = veh[k].model,
				nome = veh[k].name,
				valor = veh[k].price,
				quantidade = 1,
...
function getAllCategorieCars(id)
	local _id = id
    local result = MySQL.Sync.fetchAll("SELECT * FROM vehicles_offroad WHERE category = @id", {["@id"] = _id})
	if result[1] ~= nil then
		local veh = result
		local vehicles_offroad = {}
...
function getAllCategories()
    	local result = MySQL.Sync.fetchAll("SELECT * FROM categories_offroad ", {})
		if result[1] ~= nil then
			local cat = result
			local categories = {}
...
function getCategory(id)
	local _id = id
    local result = MySQL.Sync.fetchAll("SELECT * FROM categories_offroad WHERE name = @id", {["@id"] = _id})
	if result[1] ~= nil then
...
function getCar(vehID)
    local result = MySQL.Sync.fetchAll("SELECT * FROM vehicles_offroad WHERE model = @id", {["@id"] = vehID})
       if result[1] ~= nil then
           local veh = result[1]
           return{
                   model = veh['model'],
