function CloseShop() {
    $("#carshop").hide();
    $(".container-item").html('');
    $.post('http://motorsport/fechar', JSON.stringify({}));
}
...
$(document).ready(function() {
    ...
    $(".container-item").on('click', '#comprar', function() {
        $("#carshop").hide();
        $(".container-item").html('');
        $.post('http://motorsport/comprar', JSON.stringify({ id: $(this).data('id') }));
    });

    $(".container-item-categories").on('click', '#filter', function() {
        $(".container-item").html('').fadeOut(0);
        $(".container-item").html('').fadeIn(500);
        $.post('http://motorsport/filter', JSON.stringify({ id: $(this).data('id') }));
    });
