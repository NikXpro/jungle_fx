				...
				if Config.LicenseEnable then
					ESX.TriggerServerCallback('esx_license:checkLicense', function(hasDriversLicense)
						if hasDriversLicense then
							TriggerServerEvent('SC:motorsport:GetInfo')	
						else
...
function CreateBlip()
	...
	AddTextComponentString("Legendary Motorsport")
	EndTextCommandSetBlipName(blip)
end

local infoChecked = false
RegisterNetEvent('SC:motorsport:ListInfo')
AddEventHandler('SC:motorsport:ListInfo', function(categories, vehicles, identity)
...
RegisterNetEvent('SC:motorsport:ListCategorieCars')
AddEventHandler('SC:motorsport:ListCategorieCars', function(vehicles)
	SendNUIMessage({
		filter = true,
		veiculos = vehicles
	})
...
RegisterNetEvent('SC:motorsport:GetInCar')
AddEventHandler('SC:motorsport:GetInCar', function(model, generatedPlate)
...
RegisterNUICallback('comprar', function(data, cb)
	...
	ESX.TriggerServerCallback('SC:motorsport:BuyCar', function(success)
	if success then
			TriggerServerEvent("SC:motorsport:SpawnCar", veh, generatedPlate)
		else
...
RegisterNUICallback('filter', function(data, cb)
	...
	TriggerServerEvent("SC:motorsport:GetCategorieCars", category)
end)
