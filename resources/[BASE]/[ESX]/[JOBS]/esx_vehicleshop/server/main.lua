...
TriggerEvent('esx_society:registerSociety', 'motodealer', _U('car_dealer'), 'society_motodealer', 'society_motodealer', 'society_motodealer', {type = 'private'})
...
MySQL.ready(function()
	MySQL.Async.fetchAll('SELECT * FROM categories_moto', {}, function(_categories)
		categories = _categories

		MySQL.Async.fetchAll('SELECT * FROM vehicles_moto', {}, function(_vehicles)
			vehicles = _vehicles
...
AddEventHandler('esx_vehicleshop:setVehicleOwnedPlayerId', function(playerId, vehicleProps, model, label)
	...
	if xPlayer.job.name == 'motodealer' and xTarget then
		MySQL.Async.fetchAll('SELECT id FROM motodealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = model
		}, function(result)
			if result[1] then
				local id = result[1].id

				MySQL.Async.execute('DELETE FROM motodealer_vehicles WHERE id = @id', {
					['@id'] = id
...
AddEventHandler('esx_vehicleshop:rentVehicle', function(vehicle, plate, rentPrice, playerId)
	...
	if xPlayer.job.name == 'motodealer' and xTarget then
		MySQL.Async.fetchAll('SELECT id, price FROM motodealer_vehicles WHERE vehicle = @vehicle LIMIT 1', {
			['@vehicle'] = vehicle
		}, function(result)
			if result[1] then
				...
				MySQL.Async.execute('DELETE FROM motodealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						MySQL.Async.execute('INSERT INTO rented_moto (vehicle, plate, player_name, base_price, rent_price, owner) VALUES (@vehicle, @plate, @player_name, @base_price, @rent_price, @owner)', {
							['@vehicle']     = vehicle,
							['@plate']       = plate,
...
ESX.RegisterServerCallback('esx_vehicleshop:getCommercialVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT price, vehicle FROM motodealer_vehicles ORDER BY vehicle ASC', {}, function(result)
		cb(result)
	end)
end)

ESX.RegisterServerCallback('esx_vehicleshop:buymotodealerVehicle', function(source, cb, model)
	local xPlayer = ESX.GetPlayerFromId(source)

	if xPlayer.job.name == 'motodealer' then
		local modelPrice
		...
		if modelPrice then
			TriggerEvent('esx_addonaccount:getSharedAccount', 'society_motodealer', function(account)
				if account.money >= modelPrice then
					...
					MySQL.Async.execute('INSERT INTO motodealer_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
						['@vehicle'] = model,
						['@price'] = modelPrice
					}, function(rowsChanged)
						cb(true)
				...
				MySQL.Async.execute('DELETE FROM motodealer_vehicles WHERE id = @id', {
					...
					if rowsChanged == 1 then
						TriggerEvent('esx_addonaccount:getSharedAccount', 'society_motodealer', function(account)
							...
							xPlayer.showNotification(_U('moto_sold_for', vehicleLabel, ESX.Math.GroupDigits(price)))
						end)
					end
				end)
			else
...
ESX.RegisterServerCallback('esx_vehicleshop:getRentedVehicles', function(source, cb)
	MySQL.Async.fetchAll('SELECT * FROM rented_moto ORDER BY player_name ASC', {}, function(result)
		local vehicles = {}
...
ESX.RegisterServerCallback('esx_vehicleshop:giveBackVehicle', function(source, cb, plate)
	MySQL.Async.fetchAll('SELECT base_price, vehicle FROM rented_moto WHERE plate = @plate', {
		['@plate'] = plate
	}, function(result)
		if result[1] then
			...
			MySQL.Async.execute('DELETE FROM rented_moto WHERE plate = @plate', {
				['@plate'] = plate
			}, function(rowsChanged)
				MySQL.Async.execute('INSERT INTO motodealer_vehicles (vehicle, price) VALUES (@vehicle, @price)', {
					['@vehicle'] = vehicle,
...
ESX.RegisterServerCallback('esx_vehicleshop:resellVehicle', function(source, cb, plate, model)
	...
	if xPlayer.job.name == 'motodealer' then
		...
		if not resellPrice then
			...
			MySQL.Async.fetchAll('SELECT * FROM rented_moto WHERE plate = @plate', {
				['@plate'] = plate
			}, function(result)
...
function PayRent(d, h, m)
	...
	MySQL.Async.fetchAll('SELECT owner, rent_price FROM rented_moto', {}, function(result)
		for i=1, #result, 1 do
