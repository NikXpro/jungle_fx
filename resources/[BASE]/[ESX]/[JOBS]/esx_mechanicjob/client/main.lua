			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 174.87269592285156, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
					else
...
Citizen.CreateThread(function()
	...
	AddTextComponentSubstringPlayerName("Auto Exotic")
	EndTextCommandSetBlipName(blip)
	exports['blip_info']:SetBlipInfoTitle(blip, "Auto Exotic fixing station", false)
...
function Register(Controls, ControlName, Description, Action)
    local _Keys = {
        CONTROLS = Controls
    }
...
Register('DELETE', 'DELETE', 'Mission PNJ mécano(WIP)', function()
...
Register('F6', 'F6', 'Menu interaction mécano(WIP)', function()
	if not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
